...
Т.к. наша инфраструктура полностью описана в конфигурационных файлах terraform-а и мы ее можем создать влюбой момент при помощи одной команды,
то после выполнения каждого задания мы также рекомендуем вам  выполнять terraform destroy, чтобы не тратить кредит на ресурсы GCP.
#>
$TerraformRootFolder = "$GitRootFolder\terraform"
